...
export class ThemesController {
  ...
  // GET /themes/me
  @Get('me')
  async getMyTheme(@CurrentUser() jwt: any) {
  ...
  // PATCH /themes
  @Patch()
  async updateMyTheme(@CurrentUser() jwt: any, @Body() dto: UpdateThemeDto) {
    const email = jwt?.email;
    if (!email) throw new BadRequestException('No email in token');
    return await this.themesService.updateByEmail(email, { themeId: dto.themeId, colorHex: dto.colorHex });
  }
}
